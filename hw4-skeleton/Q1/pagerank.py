...
def author():                                                                                             
        return "lpant3" # replace gburdell3 with your Georgia Tech username.
                                                                                              
def gtid():                                                                                               
    return 903543078 # replace with your GT ID number
...
class PageRank:
    ...
    def run_pagerank(self, node_weights,  damping_factor=0.85, iterations=10):
        ...
        ### Implement your code here
        #############################################
                source_degree = self.node_degree[source]
                new_pr_values[target] += pr_values[source] / source_degree

            random_jump_pr_value = (1 - damping_factor) * node_weights[target]
            new_pr_values = [random_jump_pr_value + (damping_factor * new_pr_value) for new_pr_value in new_pr_values]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="sample command: python pagerank.py -i 5 -d 0.85 simplified_pagerank network.tsv")
