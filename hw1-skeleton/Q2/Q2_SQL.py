...
class HW2_sql():
    ...
    def part_aii_1(self,connection,path):
        ...
        with open(path, 'r') as csvfile:
            ...
            for row in file:
                connection.execute("INSERT INTO movies VALUES (?,?, ?)", tuple(row))
                connection.commit()
       ######################################################################
    ...
    def part_aii_2(self,connection, path):
        ...
        with open(path, 'r') as csvfile:
            file = csv.reader(csvfile)
            for row in file:
                connection.execute("INSERT INTO movie_cast VALUES (?,?, ?, ?, ?)", tuple(row))
                connection.commit()
        ######################################################################
    ...
    def part_aiii(self,connection):
        ...
        ############### CREATE IMPORT CODE BELOW ############################
        # Be sure that when you insert into the new cast_bio that the values are unique.
        part_aiii_insert_sql = "INSERT INTO cast_bio(cast_id, cast_name, birthday, popularity) " \
                               "SELECT DISTINCT mc.cast_id, mc.cast_name, mc.birthday, mc.popularity FROM movie_cast mc;"
        ######################################################################
    ...
    def part_c(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_c_sql = r"""SELECT printf('%.2f', COUNT(*) * 100.0 / (SELECT COUNT(*) FROM movies)) FROM movies WHERE score > 50 AND LOWER(title) LIKE '%war%' """
        ######################################################################
    ...
    def part_d(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_d_sql = "SELECT cast_name, COUNT(*) AS appearance_count FROM movie_cast GROUP BY cast_name HAVING popularity > 10 ORDER BY appearance_count DESC, cast_name LIMIT 5"
        ######################################################################
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = "SELECT movie_title, printf('%.2f',movie_score), COUNT(*) as cast_count FROM (" \
                "SELECT m.title AS movie_title, m.score AS movie_score FROM movies m JOIN movie_cast mc on m.id = mc.movie_id)" \
                "GROUP BY movie_title, movie_score ORDER BY movie_score DESC, cast_count, movie_title LIMIT 5"
        ######################################################################
    ...
    def part_f(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_f_sql = "SELECT cast_id, cast_name, average_score FROM ( " \
                     "SELECT cast_id, cast_name, printf('%.2f',AVG(score)) AS average_score, COUNT(movie_id) AS appearance_count FROM ( " \
                "SELECT cast_id, cast_name, score, movie_id FROM movies m JOIN movie_cast mc " \
                "ON m.id = mc.movie_id WHERE score >= 25) " \
                "GROUP BY cast_id, cast_name HAVING appearance_count > 2) ORDER BY average_score DESC,cast_name LIMIT 10"
        ######################################################################
    ...
    def part_g(self,connection):
        ...
        part_g_sql = "CREATE VIEW good_collaboration AS " \
                        "SELECT cast_member_id1, cast_member_id2, COUNT(movie_id) AS movie_count, printf('%.2f', AVG(score)) AS average_movie_score FROM (" \
                        "SELECT mc1.cast_id AS cast_member_id1, mc2.cast_id AS cast_member_id2, m.id AS movie_id, m.score AS score " \
                        "FROM movie_cast mc1 JOIN movie_cast mc2 ON mc1.movie_id = mc2.movie_id " \
                        "JOIN movies m ON m.id = mc1.movie_id WHERE mc1.cast_id < mc2.cast_id) " \
                        "GROUP BY cast_member_id1,cast_member_id2 HAVING movie_count >= 3 AND average_movie_score >= 40"
        ######################################################################
        return self.execute_query(connection, part_g_sql)
    
    def part_gi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_i_sql = "SELECT distinct clb.cast_id as cast_id, mc.cast_name as cast_name, clb.collaboration_score as collaboration_score FROM movie_cast mc " \
                    "JOIN (" \
                    "SELECT cast_id, printf('%.2f', AVG(average_movie_score)) as collaboration_score FROM (" \
                    "SELECT col1.cast_member_id1 as cast_id, col1.average_movie_score as average_movie_score " \
                    "FROM good_collaboration col1 UNION ALL " \
                    "SELECT col2.cast_member_id2 as cast_id, col2.average_movie_score as average_movie_score " \
                    "FROM good_collaboration col2) " \
                    "GROUP BY cast_id" \
                    ") clb ON mc.cast_id = clb.cast_id ORDER BY clb.collaboration_score desc, mc.cast_name LIMIT 5"
        ######################################################################
    ...
    def part_h(self,connection,path):
        ...
        with open(path, 'r') as csvfile:
            file = csv.reader(csvfile)
            for row in file:
                connection.execute("INSERT INTO movie_overview VALUES (?,?)", tuple(row))
                connection.commit()
        ######################################################################
